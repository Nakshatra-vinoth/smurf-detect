...
export function analyzeTransactions(transactions: Transaction[]): {
  ...
  for (const [address, wallet] of walletMap) {
    ...
    // Classify wallet role
    wallet.walletRole = classifyWalletRole(wallet, outCount, inCount);
  }
...
/**
 * Classify wallet role based on transaction patterns
 * - Mule: Receives from few sources, sends to many destinations (fan-out)
 * - Aggregator: Receives from many sources, sends to few destinations (fan-in)
 * - Splitter: Receives from few sources, sends to many destinations with decreasing values (peeling)
 */
function classifyWalletRole(
  wallet: WalletNode,
  outCount: number,
  inCount: number
): 'mule' | 'aggregator' | 'splitter' | 'normal' {
  // Mule: High fan-out, low fan-in, receives from few sources
  if (outCount >= FAN_OUT_THRESHOLD && inCount < FAN_IN_THRESHOLD && wallet.totalReceived > 0) {
    return 'mule';
  }
  
  // Aggregator: High fan-in, low fan-out, sends to few destinations
  if (inCount >= FAN_IN_THRESHOLD && outCount < FAN_OUT_THRESHOLD && wallet.totalSent > 0) {
    return 'aggregator';
  }
  
  // Splitter: High fan-out with peeling chain pattern
  if (outCount >= FAN_OUT_THRESHOLD && wallet.isPeelingSource) {
    return 'splitter';
  }
  
  // If high fan-out but no peeling, still likely a splitter
  if (outCount >= FAN_OUT_THRESHOLD && outCount > inCount * 2) {
    return 'splitter';
  }
  
  // If high fan-in but no clear aggregator pattern, check ratio
  if (inCount >= FAN_IN_THRESHOLD && inCount > outCount * 2) {
    return 'aggregator';
  }
